...
func returnPermissionResult(isGranted: Bool){
    DispatchQueue.main.async(execute: {
        if (isGranted){
            MoodTracker.webView.evaluateJavaScript("this.dispatchEvent(new CustomEvent('push-permission-request', { detail: 'granted' }))")
        }
        else {
            MoodTracker.webView.evaluateJavaScript("this.dispatchEvent(new CustomEvent('push-permission-request', { detail: 'denied' }))")
        }
    })
}
...
func checkViewAndEvaluate(event: String, detail: String) {
    if (!MoodTracker.webView.isHidden && !MoodTracker.webView.isLoading ) {
        DispatchQueue.main.async(execute: {
            MoodTracker.webView.evaluateJavaScript("this.dispatchEvent(new CustomEvent('\(event)', { detail: \(detail) }))")
        })
    }
    else {
